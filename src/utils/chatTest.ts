...
import { Stomp, Client, IMessage, StompSubscription, CompatClient } from '@stomp/stompjs';
...
class ChatSocketClient {
  ...
  connect(callback?: Callback): void {
    ...
    this.stompClient = new Client({
      webSocketFactory: () => {
        const url = `https://marketduck.server-su.site/ws-chat?token=${token.replace(/^Bearer\s+/i, '')}`;
  ...
  // 메시지 전송 (WebSocket)
  sendMessage(chatRoomId: string, sessionId: string, senderId: string, content: string, messageType: string): void {
    if (!this.connected || !this.stompClient) {
      console.error('웹소켓이 연결되어 있지 않습니다.');
      return;
    }
